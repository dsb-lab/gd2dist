...
std::vector<double> gdposteriormodel::prior(std::vector<double>& uniform){
    ...
    for(int i = 1; i < K; i++){
        transformed[K+i] = transformed[K+i-1]*std::pow(uniform[K+i],1.0/(K-i))+dataMin;
    }
    //Std
    for(int i = 0; i < K; i++){
    ...
    for(int i = 1; i < Kc; i++){
        transformed[3*K+Kc+i] = transformed[3*K+Kc+i-1]*std::pow(uniform[3*K+Kc+i],1.0/(Kc-i))+dataMin;
    }
    //Std
    for(int i = 0; i < Kc; i++){
